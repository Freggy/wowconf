...
do
	...
	function updateFunc:Health()
		value = addon.testMode and self.HealthBar:GetValue() or self.HealthBar.currValue
	...
	function updateFunc:Power()
		value = addon.testMode and self.ManaBar:GetValue() or self.ManaBar.currValue
...
addon.anchorFrame = addon.anchorFrame or CreateFrame("Frame", nil, _G.UIParent, "BackdropTemplate")
...
local UpdateBuffs do
	...
	local function ShouldShowBuff(id, nameplate, caster, isBoss, duration)
		return id and (caster == "player" and duration < 60 or isBoss)
	end
...
local function BossFrames_SetStyle()
	...
	for i = 1, MAX_BOSS_FRAMES do
		...
		local frameBorder = CreateFrame("Frame", nil, boss, "BackdropTemplate")
		...
		if BF.percent.enabled then
			...
			local frame = CreateFrame("Frame", nil, boss, "BackdropTemplate")
			frames[i] = frame
			...
			local text = frame:CreateFontString(nil, "ARTWORK", "SystemFont_Med3")
...
function addon:Enable()
	...
	anchorFrame:Hide()
	if db.anchor and db.anchor[1] and db.anchor[4] and db.anchor[5] then
		anchorFrame:ClearAllPoints()
		anchorFrame:SetPoint(unpack(db.anchor))
		self.bossFrame:ClearAllPoints()
		self.bossFrame:OrgSetPoint(unpack(db.anchor))
	else
		anchorFrame:ClearAllPoints()
		anchorFrame:SetPoint(unpack(self.defaultAnchor))
	end
	if db.scale then
		anchorFrame:SetScale(db.scale)
		BossFrames_SetScale(db.scale)
	end
...
function addon:SetMode(mode, index)
	mode = mode or "Health"
	for i = 1, #frames do
...
function anchorFrame:OnMouseWheel(delta)
	if delta < 0 then
		commands.scale(addon, (db.scale or addon.defaultScale) - BF.scale_delta)
	else
		commands.scale(addon, (db.scale or addon.defaultScale) + BF.scale_delta)
	end
end
...
function anchorFrame:OnUpdate(elapsed)
	if not InCombatLockdown() and self.moving then
		db.anchor = {self:GetPoint(1)}
		addon.bossFrame:ClearAllPoints()
		addon.bossFrame:OrgSetPoint(unpack(db.anchor))
	end
end

function anchorFrame:OnShow()
	if not self.bkgndFrame then
		self.bkgndFrame = CreateFrame("Frame", nil, self, "BackdropTemplate")
...
function buff_prototype:SetStyle()
	self.cooldown:SetScale(1 / (db.scale or BF.scale))
	self.cooldown:ClearAllPoints()
	self.cooldown:SetPoint("LEFT", self.cooldown:GetParent(), "RIGHT", BF.buff.offset.x + ((BF.percent.enabled and 53 or 0) * (db.scale or BF.scale)) + (self.index - 1) * (BF.buff.size.W + 2), BF.buff.offset.y)
...
function commands:sb(arg)
	DEFAULT_CHAT_FRAME:AddMessage(
		"Simple Boss Frame " .. GetAddOnMetadata(ADDON_NAME, "Version") .. "\n" ..
		"  /sb test\n" ..
		"  /sb reset\n" ..
		"  /sb health\n" ..
		"  /sb power\n" ..
		"  /sb mana\n" ..
		"  /sb auras: "..tostring(db.auras and "Disabled" or "Enabled"),
		1, 1, 0
	)
end
...
function commands:scale(arg)
	...
	local anchor = db.anchor or {unpack(self.defaultAnchor)}
...
function commands:aura()
	db.auras = not db.auras
	DEFAULT_CHAT_FRAME:AddMessage("Simple Boss Frame "..GetAddOnMetadata(ADDON_NAME, "Version").."\n Auras: "..(db.auras and "Disabled" or "Enabled"), 1, 1, 0)
end
