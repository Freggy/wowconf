...
local statLabelMap = {
	...
	[CR_CRIT_SPELL]=function() return format(PAPERDOLLFRAME_TOOLTIP_FORMAT, STAT_CRITICAL_STRIKE) end,
	[CR_VERSATILITY_DAMAGE_DONE]=function() return format(PAPERDOLLFRAME_TOOLTIP_FORMAT, STAT_VERSATILITY) end,
	[CR_HASTE_SPELL]=function() return format(PAPERDOLLFRAME_TOOLTIP_FORMAT, STAT_HASTE) end
}
...
function addon.tsv:OnTooltip(ev,tooltip,...)
	...
	if ( addon.tsv.db.global.showItemTooltips and ev == "OnTooltipSetItem" ) then
				...
				if ( text and text ~= "") then
					for statId, pattern in pairs(patterns) do
						...
						if ( amount ) then
							...
							local trueText = text:sub(1,e) .. " |cff"..hexStr.."(" .. tostring(trueAmount) .. ")|r" .. text:sub(e+1);
		...
		_G["FRM"]=tooltip;
--[[
-- 		if ( tt_text and tooltip.GetItem and tooltip:GetItem() ) then
-- 			local name, link = tooltip:GetItem()
-- 			local stats = {};
...
function addon.tsv:AddTrueStatValuesTooltip(tooltip,statId)
	...
	local barLabel = tostring(statInfo.bracketRating) .. "/" .. tostring(statInfo.bracketMaxRating) .. " [" .. pctLabel .. " penalty]";	
	local r,g,b = addon.tsv.db.global.fontColor.r,addon.tsv.db.global.fontColor.g,addon.tsv.db.global.fontColor.b;
