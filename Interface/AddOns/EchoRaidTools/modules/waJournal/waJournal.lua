...
local function createContentFrame()
	...
	local scrollBar = CreateFrame("Slider",nil,scrollFrame,"EncounterJournalScrollBarTemplate");
...
local function doElvUISkin(E,S,tabButton)
	...
	tabButton:Point('TOPRIGHT', _G.EncounterJournal, 'BOTTOMRIGHT', -10, E.PixelMode and 0 or 2);

	EncounterInfo.modelTab:ClearAllPoints();

	--Hackery around SetPoint being nooped
	tabButton.SetPoint(EncounterInfo.modelTab, 'RIGHT', tabButton, 'LEFT', E:Scale(-4), 0);

	tabButton:StripTextures();
	tabButton:SetText("Echo WAs");
	tabButton:GetFontString():FontTemplate(nil, nil, '');
	tabButton:SetTemplate();
	tabButton:SetScript('OnEnter', E.noop);
	tabButton:SetScript('OnLeave', E.noop);
	tabButton:Size(tabButton:GetFontString():GetStringWidth()*1.5, 20);
	tabButton.SetPoint = E.noop;

	S:HandleScrollBar(contentFrame.scrollframe.ScrollBar, 4);
...
local function createJournalTab()
	...
	unSelectedTexture:SetSize(40,40);
...
local function fixUpNestedPreview(paternMatch,journalID,waId)
	local targetData;
	...
	for i,child in ipairs(targetData.children) do
		local match = child:match(paternMatch);
		if(match)then
			local bossIndex = tonumber(match);
			if(bossIndex)then
	...
	if(addonName == "Blizzard_EncounterJournal")then
		if (not IsAddOnLoaded("!!!EchoInternal")) then
			...
			if(not playerEnteringWorldFired)then
				print("|cffff0000EchoRaidToolsError: Some addon force loaded 'Blizzard_EncounterJournal'. This will prevent some features of EchoRaidTools functioning correctly (ElvUI skinning, expanded WAs from group view. Contact developers of EchoRaidTools to debug this!")
			else
				fixUpNestedPreview("^%d+",1190,"Echo - Castle Nathria S4 - Custom");
				fixUpNestedPreview("^%s?SoD%s%-%s(%d+)",1193,"Echo - Sanctum of Domination S4 - Custom");
			end
